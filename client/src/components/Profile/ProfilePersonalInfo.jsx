...
const ProfilePersonalInfo = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`http://localhost:8000/user/${currentUser.id}`, formData, {
        headers: { Authorization: `Bearer ${currentUser.token}` }
      });
      setSuccess(response.data.message);
      setIsEditing(false);
      // Optionally update Redux store with new user data here
    } catch (err) {
...
const InfoCard = ({ icon, title, value, editable = false, name, type = "text", onChange }) => {
  return (
    <motion.div 
      whileHover={{ y: -2 }}
