...
module.exports = {
    ...
    approveUser: async (req, res, next) => {
        ...
        try {
            ...
            if (user.role !== 'seller' && user.role !== 'delivery') {
                return next(e.errorHandler(400, "Only sellers and delivery persons can be approved"));
            }

            if (user.status === 'approved') {
                return next(e.errorHandler(400, "User is already approved"));
            }
    ...
    rejectUser: async (req, res, next) => {
        ...
        try {
            ...
            if (!reason) {
                return next(e.errorHandler(400, "Rejection reason is required"));
            }
            ...
            if (user.role !== 'seller' && user.role !== 'delivery') {
                return next(e.errorHandler(400, "Only sellers and delivery persons can be rejected"));
            }

            if (user.status === 'rejected') {
                return next(e.errorHandler(400, "User is already rejected"));
            }

            user.status = 'rejected';
            user.isActive = false;

            // Set the rejection reason based on role
            if (user.role === 'seller') {
                user.rejectionReason = reason;
            } else if (user.role === 'delivery') {
                user.reasonForRejection = reason;
            }
            ...
            res.status(200).json({
                ...
                user: {
                    ...
                    status: user.status,
                    isActive: user.isActive,
                    rejectionReason: user.rejectionReason || user.reasonForRejection
                }
            });
        } catch (error) {
            next(error);
